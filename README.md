...
Артефакты собираются в CI и лежат в [релизе `latest`](https://github.com/farwydi/vultr-rulesets/releases/latest)
(в `main` их нет — только исходники):

| Файл | Формат | Назначение |
|------|--------|------------|
| [`vultr.srs`](https://github.com/farwydi/vultr-rulesets/releases/latest/download/vultr.srs)   | binary | скомпилированный rule-set для sing-box (для подключения) |
| [`vultr.json`](https://github.com/farwydi/vultr-rulesets/releases/latest/download/vultr.json) | source | человекочитаемый исходник rule-set |
